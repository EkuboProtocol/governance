...
contract StarknetStakingProxy {
    ...
    function stake(address delegate, uint128 amount) external payable onlyOwner returns (bytes32, uint256) {
        ...
        (bytes32 messageHash, uint256 messageFee) =
            l2MessageBridge.sendMessageToL2{value: msg.value}(l2StakingProxy, L1_HANDLER_SELECTOR, payload);
    ...
    function withdraw(address delegate, address recipient, uint128 amount)
        external
        payable
        onlyOwner
        returns (bytes32, uint256)
    ...
    function emergencyTransfer(address token, address recipient, uint256 amount)
        external
        payable
        onlyOwner
        returns (bytes32, uint256)
