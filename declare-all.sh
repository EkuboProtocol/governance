...
declare_class_hash() {
    ...
    starkli declare --watch \
        --network "$NETWORK" \
        --keystore-password "$STARKNET_KEYSTORE_PASSWORD" \
        --casm-file "target/dev/governance_${class_name}.compiled_contract_class.json" \
        "target/dev/governance_${class_name}.contract_class.json"
}
